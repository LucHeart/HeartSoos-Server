...
namespace LucHeart.HeartSoos
{
    ...
    public abstract class WebsocketBaseController<T> : Microsoft.AspNetCore.Mvc.Controller, IAsyncDisposable, IWebsocketController<T> where T : class
    {
        /// <inheritdoc />
        ...
        public async Task Get()
        {
            if (!HttpContext.WebSockets.IsWebSocketRequest)
            {
                ...
                var response = new ProblemDetails() {Title = "Invalid request", Detail = "Request is not a websocket request", Status = StatusCodes.Status400BadRequest};
