...
public static class JsonWebSocketUtils
{
    ...
    public static async Task<OneOf.OneOf<T?, DeserializeFailed, WebsocketClosure>> ReceiveFullMessageAsyncNonAlloc<T>(
        WebSocket socket, JsonSerializerOptions serializerOptions, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                return JsonSerializer.Deserialize<T>(message.GetBuffer().AsSpan(0, bytes), serializerOptions);
            }
            catch (Exception e)
            {
